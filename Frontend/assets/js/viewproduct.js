  ...
  if (sessionStorage.getItem('fk_role') == '1') {
    ...
    document.getElementById('loadData3').innerHTML+=`
      <tr class="xyz3">
              ...
              <td class="text-center">${obj.product_price} Rs.</td>
              <td class="text-center">${new Date(Date.parse(obj.product_created_at)).toLocaleString()}</td>              
              <th scope="row" class="text-center"><button class="btn btn-danger" style="font-size: 20px;" onclick="produ_del('${obj.product_id}')"><i class='bx bx-trash'></i></button></th>
      </tr>           
    `;
    }},
    error:function(JQ){
    ...
    function produ_del(prod_id){
  
      $.ajax({
        url: `http://localhost:63626/api/Home/ProductDelete/${prod_id}`,
            type: 'Delete',
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
